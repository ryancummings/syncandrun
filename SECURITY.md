...
Each installation has one owner. Only an operator-created, single-use invitation can start initial ownership setup, and subsequent access must match that owner's Plex identity. Disconnect does not transfer ownership. A host administrator can access guest credentials and metadata; guests must trust the administrator. Independent installations are not a substitute for operating-system or network isolation against hostile guests.
...
The database contains encrypted Plex credentials and sensitive library metadata. Its encryption secret is held separately in `.env`; possession of both permits decryption. Protect backups, invitation files, host access, and signing keys. Keep `.env` out of logs and repositories.
