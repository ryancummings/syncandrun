...
Obtain the owner's target host, reviewed source revision, installation name, local port, canonical origin, and chosen ingress method. One installation has one owner and may pair multiple watches belonging to that owner. A private-IP HTTP origin requires the owner's explicit opt-in and a home-LAN boundary. For a domain-free Funnel installation the owner must authenticate Tailscale and authorize Funnel; do not reuse unrelated tunnel credentials.
...
2. Use a clean checkout of the reviewed source. Run `python3 scripts/deploy-local.py --origin <origin> --project <unique-name> --port <unused-port> --state-dir <private-directory-outside-checkout>`, adding `--allow-lan-http` only for the owner-approved private-IP HTTP route. It creates credentials once, rejects conflicting identity/resources, and reuses an existing image for the same source revision and architecture. Keep secret values out of tool output, shell tracing, chat, and version control.
...
4. Configure the chosen route as documented. Verify the exact browser and watch origin from their intended network: use home Wi-Fi for a home-LAN route, or an outside network for public ingress. For home-LAN HTTPS, confirm DNS resolves to a reachable LAN proxy; for opted-in HTTP, confirm the private-IP proxy is reachable only on the home LAN. Verify certificates for every HTTPS hop. Keep the companion bound to loopback. Never turn off TLS verification to make a watch test pass.
...
Report the source revision, local image tag, installation/project name, browser/watch origin and ingress method, health result, authentication verification result, backup/restore result, and the next concrete owner action. Distinguish executed checks from pending ones. Record whether physical-watch acceptance and audio transfer through the chosen route passed or remain unverified. Never include Plex tokens, `.env` contents, setup-link URLs, authentication cookies, or a database in the handoff.
