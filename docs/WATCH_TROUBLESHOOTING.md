...
Build and sideload using [DEVELOPMENT.md](DEVELOPMENT.md), and configure trusted HTTPS using [DEPLOYMENT.md](DEPLOYMENT.md). These procedures preserve useful development knowledge; they do not establish that the current build has passed the [physical acceptance run](VALIDATION.md).
...
If native entry is unavailable or confirmation appears stuck, use **Settings -> Advanced editor**. Up/Down selects a character or action, Select appends the character or activates SAVE/CANCEL, and Back deletes a character (or exits when empty). Keep HTTPS selected for supported deployments. **Settings -> Test companion** should report **Companion ready** before pairing.
...
|---|---|
| Companion not ready or unreachable | Check local `/health/ready`, then the configured HTTPS origin without disabling certificate validation. Readiness establishes companion startup, not Plex reachability. Verify DNS and certificate trust from the watch's network. |
...
| Artwork missing while audio works | Artwork runs after the audio commit and is optional. Retry to obtain fresh signed capabilities. Garmin's image service needs to reach the artwork origin; a hostname resolving only to a private LAN address can prevent this even when watch audio succeeds. Prefer the documented canonical HTTPS deployment. |
