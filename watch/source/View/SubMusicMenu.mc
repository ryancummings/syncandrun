	...
	module Menu {
		...
		class MenuBase {
			...
			function getItem(idx) {
				// if ($.debug) {
				// 	System.println("SyncAndRunMenu::getItem( idx: " + idx + " ) - " + d_title);
				// }
		...
		class MenuLoader {
			...
			function initialize(menu, delegate) {
				...
				if ((error != null) && (loaded == true)) {
					loadView(new TextView(error.toString()), null, WatchUi.SLIDE_IMMEDIATE);
					return;
				}
			...
			function onLoaded(error) {
				if ($.debug) {
					System.println("MenuLoader::onLoaded(error: " + error + ")");
				}
